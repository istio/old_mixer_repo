...
type fakeExecutor struct {
	...
	quotaFunc  func() (rpc.Status, *aspect.QuotaMethodResp)
}
...
func (fe fakeExecutor) Quota(ctx context.Context, requestBag *attribute.MutableBag, responseBag *attribute.MutableBag,
	qma *aspect.QuotaMethodArgs) (rpc.Status, *aspect.QuotaMethodResp) {
	return fe.quotaFunc()
}
...
func TestHandler(t *testing.T) {
	...
	for _, c := range cases {
		e := fakeExecutor{
			...
			quotaFunc: func() (rpc.Status, *aspect.QuotaMethodResp) {
				if c.errStr != "" {
					return status.WithInternal(c.errStr), nil
				}
	...
	e := fakeExecutor{
		...
		quotaFunc: func() (rpc.Status, *aspect.QuotaMethodResp) {
			return status.OK, &aspect.QuotaMethodResp{Amount: 42}
		},
	}
