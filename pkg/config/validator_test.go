...
func (f *fakeVFinder) FindBuilderInfo(name string) (*handlers.Info, bool) {
	v, found := f.hbi[name]
	return v, found
}
...
func newVfinder(ada map[string]adapter.ConfigValidator, asp map[Kind]AspectValidator,
	hbi map[string]*handlers.Info) *fakeVFinder {
	var kinds KindSet
	for k := range asp {
		kinds = kinds.Set(k)
	}
...
func TestConvertHandlerParamsErrors(t *testing.T) {
	tTable := []struct {
		...
		hndlrValidateCnfg handlers.ValidateConfigFn
		errorStr          string
	}{
		{
...
func TestValidateHandlers(t *testing.T) {
	...
	tests := []*configTable{
		...
		},
		{
			nil,
			nil,
			map[string]*handlers.Info{ /*Empty lookup. Should cause error, Adapter not found*/ },
			nil, 1, "service.name == “*”", false, ConstGlobalConfig,
		},
		{
			nil,
...
`
	...
	tests := []*configTable{
		...
		{
			hbi:     map[string]*handlers.Info{ /*Empty lookup. Should cause error, Adapter not found*/ },
			cfg:     globalConfig,
			nerrors: 1,
		},
	}
