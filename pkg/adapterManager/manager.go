...
import (
	...
	adacfg "istio.io/mixer/pkg/adapter/config"
...
type builderFinder interface {
	// FindBuilder finds a builder by name. == resolver.Adapter.Impl.
...
func (m *Manager) dispatch(ctx context.Context, requestBag, responseBag *attribute.MutableBag, cfgs []*cpb.Combined, invokeFunc invokeExecutorFunc) rpc.Status {
	...
	// TODO: look into pooling both result array and channel, they're created per-request and are constant size for resolver lifetime.
...
// Only one of the two fields is allowed to be set, the other must be nil.
type builderOrHandler struct {
	name    string
	builder adapter.Builder
	handler adacfg.Handler
}

// execute performs action described in the combined config using the attribute bag
func (m *Manager) execute(
	ctx context.Context, cfg *cpb.Combined, requestBag, responseBag *attribute.MutableBag, df descriptor.Finder, invokeFunc invokeExecutorFunc) (out rpc.Status) {
	...
	defer func() {
		if r := recover(); r != nil {
			out = status.WithError(fmt.Errorf("adapter '%s' panicked with '%v'", cfg.Builder.Name, r))
		}
	}()
...
func newCacheKey(kind config.Kind, cfg *cpb.Combined) (*cacheKey, error) {
	...
	if cfg.Builder.Params != nil {
		...
		if !ok {
			pool.PutBuffer(b)
			return nil, fmt.Errorf("non-proto resolver.Builder.Params: %v", cfg.Builder.Params)
		}
	...
	if cfg.Aspect.Params != nil {
		...
		if !ok {
			pool.PutBuffer(b)
			return nil, fmt.Errorf("non-proto resolver.Aspect.Params: %v", cfg.Aspect.Params)
		}
		if err := pbuf.Marshal(ppb); err != nil {
			pool.PutBuffer(b)
			return nil, err
