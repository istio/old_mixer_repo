...
type registry struct {
	builders map[string]map[string]adapter.Builder
}

// newRegistry returns a new Builder registry.
func newRegistry(builders []adapter.RegisterFn) *registry {
	r := &registry{make(map[string]map[string]adapter.Builder)}
...
func BuilderMap(builders []adapter.RegisterFn) map[string]map[string]adapter.Builder {
