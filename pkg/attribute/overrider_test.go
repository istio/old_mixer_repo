...
func TestOverrider(t *testing.T) {
	...
	ao.SetString("N2", "42")
	ao.SetInt64("N4", 42)
	ao.SetFloat64("N6", 42.0)
	ao.SetBool("N8", true)
	ao.SetTime("N10", t42)
	ao.SetBytes("N12", []byte{42})
	...
	// strings
	{
		...
		if r, found = ao.String("XYZ"); found {
	...
	// float64
	{
		...
		if r, found = ao.Float64("N5"); !found {
		...
		if r, found = ao.Float64("N6"); !found {
		...
		if r, found = ao.Float64("XYZ"); found {
			t.Error("XYZ was found")
		}
	}
