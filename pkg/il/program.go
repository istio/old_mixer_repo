...
// The IL is designed for a 32-bit instruction set. The opcode/argument size of the IL is 32-bits
...
//  strings are interned in a StringsTable, thus allowing extremely fast equality comparisons. The
//  id of the string in the StringsTable is encoded in the ByteCode, when stored, or during execution.
...
// to raise these errors from within the IL. However, it is not possible to trap or return these
...
// and vice-versa.
//
// The IL supports a stack-based calling-convention. All the parameters to a function needs to be
// pushed onto the stack before CALL instruction is invoked. Similarly, the return values needs to be
// pushed onto the stack when RET is called.
//
...
type Program struct {
	...
	// codeEnd points to the end of the byte-codes within code.
	codeEnd uint32
}
...
func NewProgram() Program {
	...
	p := Program{
		strings:   strings,
		Functions: newFunctionsTable(strings),
		code:      make([]uint32, programCodeSize),
		codeEnd:   1,
	}

	p.code[0] = uint32(Halt)

	return p
}

// AddExternDef adds a new external/native function to the program. The actual bodies of functions
// needs to be supplied to the interpreter separately.
...
func (p *Program) AddFunction(name string, parameters []Type, returnType Type, body []uint32) error {
	start := p.codeEnd + 1 // Leave a single instruction gap between function bodies.
	l := uint32(len(body))

	// Rewrite jump addresses from local to global when transcribing
	var i uint32
	for i = 0; i < l; {
		op := Opcode(body[i])
		info := opCodeInfos[op]
...
func (p *Program) Strings() *StringsTable {
	return p.strings
}
