...
const (
	...
	// ILoadS loads its string argument into the target register.
	ILoadS Opcode = 30

	// ILoadB loads its boolean argument into the target register.
	ILoadB Opcode = 31

	// ILoadI loads its integer argument into the target register.
	ILoadI Opcode = 32

	// ILoadD loads its double argument into the target register.
	ILoadD Opcode = 33

	// IPushS pushes its string argument to the stack.
	IPushS Opcode = 40

	// IPushB pushes its boolean argument to the stack.
	IPushB Opcode = 41

	// IPushI pushes its integer argument to the stack.
	IPushI Opcode = 42

	// IPushD pushes its double argument to the stack.
	IPushD Opcode = 43
	...
	// IEqS pops a string value from the stack and compare it against its argument for equality.
	// If equal, then it pushes 1 into the stack, otherwise it pushes 0.
	IEqS Opcode = 70

	// IEqB pops a bool value from the stack and compare it against its argument for equality.
	// If equal, then it pushes 1 into the stack, otherwise it pushes 0.
	IEqB Opcode = 71

	// IEqI pops an integer value from the stack and compare it against its argument for equality
	// If equal, then it pushes 1 into the stack, otherwise it pushes 0.
	IEqI Opcode = 72

	// IEqD pops a double value from the stack and compare it against its argument for equality
	// If equal, then it pushes 1 into the stack, otherwise it pushes 0.
	IEqD Opcode = 73
	...
	// IXor pops a boolean value from the stack and performs logical exclusive-or with its
	// argument, then pushes the result back into stack.
	IXor Opcode = 83

	// IAnd pops a boolean value from the stack and performs logical and with its
	// argument, then pushes the result back into stack.
	IAnd Opcode = 84

	// IOr pops a boolean value from the stack and performs logical or with its
	// argument, then pushes the result back into stack.
	IOr Opcode = 85
	...
	// ResolveR lookups up a record attribute value in the bag, with the given name.
	// If successful, pushes the resolved record into stack, otherwise raises error.
	ResolveR Opcode = 94
	...
	// TResolveR lookups up a record attribute value in the bag, with the given name.
	// If successful, pushes the resolved record value, then 1 into the stack,
	// otherwise pushes 0.
	TResolveR Opcode = 104
	...
	// IAddI pops an integer value from the stack, adds the popped value and its argument,
	// and pushes the result back into stack. The operation follows Go's integer addition
	// semantics.
	IAddI Opcode = 114

	// IAddD pops a double value from the stack, adds the popped value and its argument,
	// and pushes the result back into stack. The operation follows Go's double addition
	// semantics.
	IAddD Opcode = 115

	// ISubI pops an integer value from the stack, subtracts its argument from the popped value,
	// then pushes the result back into stack.  The operation follows Go's integer subtraction
	// semantics.
	ISubI Opcode = 116

	// ISubD pops a double value from the stack, subtracts its argument from the popped value,
	...
	// Lookup pops a string, then a record from the stack and perform a lookup on the record
	// using the string as the name. If a value is found, then the value is pushed into the
	// stack.  Otherwise raises an error.
	Lookup Opcode = 210

	// TLookup pops a string, then a record from the stack and perform a lookup on the record
	// using the string as the name. If a value is found, then the value is pushed into the
	// stack, then 1.  Otherwise 0 is pushed to into the stack.
	TLookup Opcode = 211

	// ILookup pops a record from the stack and perform a lookup on the record using the string
...
var opCodeInfos = map[Opcode]opcodeInfo{
	...
	// ILoadS loads its string argument into the target register.
	ILoadS: {name: "ILoadS", keyword: "iload_s", args: []OpcodeArg{
		// The target register.
		OpcodeArgRegister,
		// The string to load.
		OpcodeArgString,
	...
	// ILoadB loads its boolean argument into the target register.
	ILoadB: {name: "ILoadB", keyword: "iload_b", args: []OpcodeArg{
		// The target register.
		OpcodeArgRegister,
		// The bool to load.
		OpcodeArgBool,
	...
	// ILoadI loads its integer argument into the target register.
	ILoadI: {name: "ILoadI", keyword: "iload_i", args: []OpcodeArg{
		// The target register.
		OpcodeArgRegister,
		// The integer to load.
		OpcodeArgInt,
	...
	// ILoadD loads its double argument into the target register.
	ILoadD: {name: "ILoadD", keyword: "iload_d", args: []OpcodeArg{
		// The target register.
		OpcodeArgRegister,
		// The double to load.
		OpcodeArgDouble,
	...
	// IPushS pushes its string argument to the stack.
	IPushS: {name: "IPushS", keyword: "ipush_s", args: []OpcodeArg{
		// The string to push.
		OpcodeArgString,
	}},

	// IPushB pushes its boolean argument to the stack.
	IPushB: {name: "IPushB", keyword: "ipush_b", args: []OpcodeArg{
		// The boolean to push.
		OpcodeArgBool,
	}},

	// IPushI pushes its integer argument to the stack.
	IPushI: {name: "IPushI", keyword: "ipush_i", args: []OpcodeArg{
		// The integer to push.
		OpcodeArgInt,
	}},

	// IPushD pushes its double argument to the stack.
	IPushD: {name: "IPushD", keyword: "ipush_d", args: []OpcodeArg{
		// The double to push.
		OpcodeArgDouble,
	}},
	...
	// IEqS pops a string value from the stack and compare it against its argument for equality.
	// If equal, then it pushes 1 into the stack, otherwise it pushes 0.
	IEqS: {name: "IEqS", keyword: "ieq_s", args: []OpcodeArg{
	...
	// IEqB pops a bool value from the stack and compare it against its argument for equality.
	// If equal, then it pushes 1 into the stack, otherwise it pushes 0.
	IEqB: {name: "IEqB", keyword: "ieq_b", args: []OpcodeArg{
	...
	// IEqI pops an integer value from the stack and compare it against its argument for equality.
	// If equal, then it pushes 1 into the stack, otherwise it pushes 0.
	IEqI: {name: "IEqI", keyword: "ieq_i", args: []OpcodeArg{
	...
	// IEqD pops a double value from the stack and compare it against its argument for equality.
	// If equal, then it pushes 1 into the stack, otherwise it pushes 0.
	IEqD: {name: "IEqD", keyword: "ieq_d", args: []OpcodeArg{
	...
	// IXor pops a boolean value from the stack and performs logical exclusive-or with its
	// argument, then pushes the result back into stack.
	IXor: {name: "IXor", keyword: "ixor", args: []OpcodeArg{
	...
	// IAnd pops a boolean value from the stack and performs logical and with its
	// argument, then pushes the result back into stack.
	IAnd: {name: "IAnd", keyword: "iand", args: []OpcodeArg{
	...
	// IOr pops a boolean value from the stack and performs logical or with its
	// argument, then pushes the result back into stack.
	IOr: {name: "IOr", keyword: "ior", args: []OpcodeArg{
	...
	// ResolveR lookups up a record attribute value in the bag, with the given name.
	// If successful, pushes the resolved record into stack, otherwise raises error.
	ResolveR: {name: "ResolveR", keyword: "resolve_r", args: []OpcodeArg{
		// The name of the attribute.
		OpcodeArgString,
	}},
	...
	// ResolveB lookups up a  ool attribute value in the bag, with the given name.
	// If successful, pushes the resolved bool value, then 1 into the stack,
	// otherwise pushes 0.
	...
	// If successful, pushes the resolved record value, then 1 into the stack,
	// otherwise pushes 0.
	TResolveR: {name: "TResolveR", keyword: "tresolve_r", args: []OpcodeArg{
		// The name of the attribute.
		OpcodeArgString,
	}},
	...
	// and pushes the result back into stack. The operation follows Go's integer addition
	// semantics.
	IAddI: {name: "IAddI", keyword: "iadd_i", args: []OpcodeArg{
		// Value to add
		OpcodeArgInt,
	}},
	...
	// and pushes the result back into stack. The operation follows Go's double addition
	// semantics.
	IAddD: {name: "IAddD", keyword: "iadd_d", args: []OpcodeArg{
		// Value to add
		OpcodeArgDouble,
	}},
	...
	// then pushes the result back into stack.  The operation follows Go's integer subtraction
	// semantics.
	ISubI: {name: "ISubI", keyword: "isub_i", args: []OpcodeArg{
		// Value to subtract
		OpcodeArgInt,
	}},
	...
	// then pushes the result back into stack.  The operation follows Go's double subtraction
	// semantics.
	ISubD: {name: "ISubD", keyword: "isub_d", args: []OpcodeArg{
		// Value to subtract
		OpcodeArgDouble,
	}},
	...
	// Jz pops a bool value from the stack. If the value is not 0, then jumps to the given
	// instruction address.
	...
	// RLookup pops a string, then a record from the stack and perform a lookup on the record
	// using the string as the name. If a value is found, then the value is pushed into the
	// stack, then 1.  Otherwise 0 is pushed to into the stack.
	...
	// parameter as the name. If a value is found, then the value is pushed into the stack
	// Otherwise raises an error.
	ILookup: {name: "ILookup", keyword: "ilookup", args: []OpcodeArg{
		// The name of the attribute.
		OpcodeArgString,
	}},
}
