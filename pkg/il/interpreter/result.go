...
// Result contains the result of an evaluation performed by the compiler.
type Result struct {
	s  *il.StringsTable
	t  il.Type
	v1 uint32
...
// Type returns the underlying type of the result.
...
// Integer returns the value contained in the result as an integer. If the underlying result is not integer,
// it panics.
...
// Double returns the value contained in the result as a double. If the underlying result is not double,
// it panics.
...
func (r Result) Interface() interface{} {
	...
	case il.Void:
		return nil
	default:
		panic(fmt.Sprintf("Unknown type: %v", r.t))
	}
}
