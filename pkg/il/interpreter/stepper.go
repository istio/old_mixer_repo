...
// Stepper executes a program using single steps.
type Stepper struct {
	i *Interpreter
...
// NewStepper returns a new stepper, that executes the given program and externs.
...
// Begin starts stepping into the program.
...
// Done indicates that the stepper is done, and program is completed.
...
func (s *Stepper) String() string {
	...
	b.WriteString("code:   [\n")
	il.WriteFn(&b, s.program.ByteCode(), s.fn, s.program.Strings(), s.ip)
	b.WriteString("]\n\n")
