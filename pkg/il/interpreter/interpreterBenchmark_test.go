...
func BenchmarkIL(b *testing.B) {
	for n, bt := range interpreterBenchmarkTests {
		finder := descriptor.NewFinder(&bt.program.config)
		p, err := compiler.Compile(bt.program.expression, finder)
		if err != nil {
			b.Fatal(err)
		}
		if len(bt.program.code) != 0 {
			txt := il.WriteText(p)
			if strings.TrimSpace(bt.program.code) != strings.TrimSpace(txt) {
				b.Fatalf("unexpected program output: \n%s\n", txt)
			}
