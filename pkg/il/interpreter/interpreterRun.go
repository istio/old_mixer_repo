...
func (in *Interpreter) run(fn *il.Function, bag attribute.Bag, step bool) (Result, error) {
	...
	if len(fn.Parameters) != 0 {
		tErr = fmt.Errorf("init function must have 0 args")
		goto RETURN_ERR
	}
	...
	for {
		...
		case il.Halt:
			tErr = fmt.Errorf("catching fire as instructed")
			goto RETURN_ERR
		...
		case il.Err:
			t3 = body[ip]
			ip++
			tStr = strings.GetString(t3)
			tErr = fmt.Errorf(tStr)
			goto RETURN_ERR
		...
		case il.ILoadS, il.ILoadB:
			t1 = body[ip]
			ip++
			registers[t1] = body[ip]
			ip++

		case il.ILoadI, il.ILoadD:
			t1 = body[ip]
			ip++
			registers[t1] = body[ip]
...
STACK_OVERFLOW:
	tErr = fmt.Errorf("stack overflow")
	goto RETURN_ERR
STACK_UNDERFLOW:
	tErr = fmt.Errorf("stack underflow")
	goto RETURN_ERR
INVALID_HEAP_ACCESS:
	tErr = fmt.Errorf("invalid heap access")
	goto RETURN_ERR
HEAP_OVERFLOW:
	tErr = fmt.Errorf("heap overflow")
	goto RETURN_ERR
