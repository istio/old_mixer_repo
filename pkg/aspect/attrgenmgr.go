...
func (attrGenMgr) ValidateConfig(c config.AspectParams, v expr.TypeChecker, df descriptor.Finder) (cerrs *adapter.ConfigErrors) {
	params := c.(*apb.AttributesGeneratorParams)
	for n, expr := range params.InputExpressions {
		if _, err := v.EvalType(expr, df); err != nil {
