...
func validateTemplateExpressions(ceField string, expressions map[string]string, v expr.TypeChecker, df expr.AttributeDescriptorFinder) (
	ce *adapter.ConfigErrors) {
	...
	for name, exp := range expressions {
		if _, err := v.EvalType(exp, df); err != nil {
