...
func TestQuotas_DescToDef(t *testing.T) {
	...
	for idx, c := range cases {
		t.Run(strconv.Itoa(idx), func(t *testing.T) {
			result, err := quotaDescToDef(c.in)
			...
			if !strings.Contains(errString, c.errString) {
				t.Errorf("quotaDescToDef(%v) = _, %v; wanted err containing %s", c.in, err, c.errString)
			}
			if !reflect.DeepEqual(result, c.out) {
				t.Errorf("quotaDescToDef(%v) = %v, %v; wanted %v", c.in, result, err, c.out)
			}
		})
	}
}
...
func TestQuotas_Find(t *testing.T) {
	cases := []struct {
		...
		{[]*aconfig.QuotasParams_Quota{{Descriptor_: "foo"}}, "foo", true},
		{[]*aconfig.QuotasParams_Quota{{Descriptor_: "bar"}}, "foo", false},
	}
	for _, c := range cases {
