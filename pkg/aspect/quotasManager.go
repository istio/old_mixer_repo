...
func quotaDefinitionFromProto(desc *dpb.QuotaDescriptor) (*adapter.QuotaDefinition, error) {
	...
	for name, labelType := range desc.Labels {
		...
		if err != nil {
			return nil, fmt.Errorf("descriptor '%s' label '%s' failed to convert label type value '%v' from proto : %v",
	...
	if err != nil {
		return nil, fmt.Errorf("descriptor '%s' failed to parse duration from proto : %v", desc.Name, err)
	}
