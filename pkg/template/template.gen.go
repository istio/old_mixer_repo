	...
	SupportedTmplInfo = map[string]Info{
		...
		checknothing.TemplateName: {
			...
			InferType: func(cp proto.Message, tEvalFn TypeEvalFn) (proto.Message, error) {
				...
				if cpb.Placeholder == "" {
					return nil, fmt.Errorf("expression for field Placeholder cannot be empty")
				}

				if t, e := tEvalFn(cpb.Placeholder); e != nil || t != istio_mixer_v1_config_descriptor.STRING {
					if e != nil {
						return nil, fmt.Errorf("failed to evaluate expression for field Placeholder: %v", e)
					}
					return nil, fmt.Errorf("error type checking for field Placeholder: Evaluated expression type %v want %v", t, istio_mixer_v1_config_descriptor.STRING)
				}
