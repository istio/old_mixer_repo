...
func (mq *memQuota) alloc(args adapter.QuotaArgs, bestEffort bool) (int64, error) {
	return mq.common(args, func(d *adapter.QuotaDefinition, key string) int64 {
		...
		if d.ExpirationSeconds == 0 {
			...
			if !ok {
				window = newRollingWindow(d.MaxAmount, d.ExpirationSeconds*ticksPerSecond)
...
func (mq *memQuota) ReleaseBestEffort(args adapter.QuotaArgs) (int64, error) {
	return mq.common(args, func(d *adapter.QuotaDefinition, key string) int64 {
		...
		if d.ExpirationSeconds == 0 {
			...
			// no support for freeing expiring quotas, they free themselves over time
			result = 0
		}
