...
type memQuota struct {
	...
	// the counters we track for allocation quotas, protected by lock
	cells map[string]int64

	// the rolling windows we track for rate quotas, protected by lock
...
func newAspect(env adapter.Env, c *config.Params, d map[string]*adapter.QuotaDefinition) (adapter.QuotaAspect, error) {
	mq := &memQuota{
		definitions: d,
...
func (mq *memQuota) alloc(args adapter.QuotaArgs, bestEffort bool) (int64, error) {
	return mq.common(args, func(d *adapter.QuotaDefinition, key string) int64 {
		result := args.QuotaAmount

		if d.Kind == adapter.AllocationKind {
			// allocation quota
			...
			if !ok {
				// how many ticks per window?
				ticksInWindow := ticksPerSecond
				if d.Kind == adapter.PerMinuteKind {
					ticksInWindow = ticksPerSecond * 60
				}

				window = newRollingWindow(d.MaxAmount, int32(ticksInWindow))
...
func (mq *memQuota) ReleaseBestEffort(args adapter.QuotaArgs) (int64, error) {
	return mq.common(args, func(d *adapter.QuotaDefinition, key string) int64 {
		...
		if d.Kind == adapter.AllocationKind {
			...
			// no support for freeing rate quotas
			result = 0
		}
...
func (mq *memQuota) common(args adapter.QuotaArgs, qf quotaFunc) (int64, error) {
