...
var (
	...
	conf = &config.Params{
		MinDeduplicationWindow: &duration.Duration{Seconds: 1},
	}
)
...
func (builder) ValidateConfig(cfg adapter.AspectConfig) (ce *adapter.ConfigErrors) {
	...
	dedupWindow, err := ptypes.Duration(c.MinDeduplicationWindow)
	if err != nil {
		ce = ce.Append("MinDeduplicationWidnow", err)
	} else if dedupWindow <= 0 {
		ce = ce.Appendf("MinDeduplicationWindow", "deduplication window of %v is invalid, must be > 0", dedupWindow)
	}
...
func newAspect(env adapter.Env, c *config.Params, definitions map[string]*adapter.QuotaDefinition) (adapter.QuotasAspect, error) {
	dedupWindow, _ := ptypes.Duration(c.MinDeduplicationWindow)
