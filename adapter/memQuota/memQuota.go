...
func (mq *memQuota) Alloc(args adapter.QuotaArgs) (int64, time.Duration, error) {
	return mq.alloc(args, false)
}

func (mq *memQuota) AllocBestEffort(args adapter.QuotaArgs) (int64, time.Duration, error) {
	return mq.alloc(args, true)
}

func (mq *memQuota) alloc(args adapter.QuotaArgs, bestEffort bool) (int64, time.Duration, error) {
	return mq.commonWrapper(args, func(d *adapter.QuotaDefinition, key string, currentTime time.Time, currentTick int64) (int64, time.Time, time.Duration) {
...
func (mq *memQuota) ReleaseBestEffort(args adapter.QuotaArgs) (int64, error) {
	amount, _, err := mq.commonWrapper(args, func(d *adapter.QuotaDefinition, key string, currentTime time.Time, currentTick int64) (int64, time.Time, time.Duration) {
		result := args.QuotaAmount
